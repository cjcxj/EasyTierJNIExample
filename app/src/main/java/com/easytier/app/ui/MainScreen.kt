...
fun MainScreen(
    ...
    onRefreshDetailedInfo: suspend (Boolean) -> Unit,
    getLatestStatus: () -> EasyTierManager.EasyTierStatus?
) {
                ...
                onClick = {
                    try {
                        ...
                        if (!networkInfoJson.isNullOrBlank()) {
                            ...
                            android.widget.Toast.makeText(context, "无网络信息可复制", android.widget.Toast.LENGTH_SHORT)
                                .show()
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("MainScreen", "collectNetworkInfos 调用失败", e)
                        android.widget.Toast.makeText(context, "获取信息失败: ${e.message}", android.widget.Toast.LENGTH_LONG)
                            .show()
                    }
                },
                modifier = Modifier.fillMaxWidth(),
...
@Composable
fun DetailedInfoCard(info: DetailedNetworkInfo?,navController: NavController, onRefresh: () -> Unit) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        elevation = CardDefaults.cardElevation(defaultElevation = 4.dp)
    ) {
                ...
                Column(modifier = Modifier.heightIn(max = 400.dp)) {
                    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        items(info.finalPeerList) { peer ->
                            FinalPeerInfoItem(peer, onClick = {
                                navController.navigate("peerDetail/${peer.hostname}")
                            })
                        }
                    }
                }
